<table class="display payment-table" id="">
    ...
    <?php
        $slno = 1; 
        $customerselect = "SELECT * FROM `purchase_request` WHERE  `first_approval` = 4 OR `second_approval` = 4 ORDER BY `pur_id` DESC";
        switch ($logged_admin_role) {
            case '6':
                // user
                $customerselect = "SELECT * FROM `purchase_request` WHERE( `created_by` = '$logged_admin_id') AND (`first_approval` = 4 OR `second_approval` = 4 )   ORDER BY `pur_id` DESC";
                $editlink = "";
            break;
            case '3':
                // TL
                $customerselect = "SELECT * FROM `purchase_request` WHERE  (`team_leader` = '$logged_admin_id' OR `created_by` = '$logged_admin_id') AND  (`first_approval` = 4 OR `second_approval` = 4) ORDER BY `pur_id` DESC";
                $editlink = "";
            break;
            case '9':
                // TL
                $customerselect = "SELECT * FROM `purchase_request` WHERE  (`org_name` = '2' OR `org_name` = 'Agem')  AND  (`first_approval` = 4 OR `second_approval` = 4) ORDER BY `pur_id` DESC";
                $editlink = "";
            break;
            case '11':
                // OL
                $customerselect = "SELECT * FROM `purchase_request` WHERE (`created_by` = '$logged_admin_id' OR `org_name` = '$logged_admin_org') AND `orglead_approval` = 4 AND `first_approval` = 0 ORDER BY `pur_id` DESC";
                $editlink = "";
            break;
    ...
    ?>
        <tr> 
            <td><?php echo $newDate = date('d-M-Y h:i A', strtotime($row['cancelled_time'])); ?></td>
