<table class="display payment-table" id="exportPaymentTable">
    ...
    <tbody>
        ...
        switch ($logged_admin_role) {
            ...
            case '10':
              //Team Leader
              $customerselect = "SELECT * FROM `payment_request` WHERE (`team_leader` = '$logged_admin_id' OR `created_by` = '$logged_admin_id') ORDER BY `pay_id` DESC ";
            break;
            case '4':
              //Finance  
            ...
            case '7':
              // Purchase Team  
              $customerselect = "SELECT * FROM `payment_request` WHERE `raised_by` = 2 ORDER BY `pay_id` DESC ";
            break;
            case '1':
              //Admin  
              $customerselect = "SELECT * FROM `payment_request` ORDER BY `pay_id` DESC ";
            ...
            case '9':
              //AGEM Accounts  
              $customerselect = "SELECT * FROM `payment_request` WHERE (`org_name`=2  OR `org_name`='Agem')  ORDER BY `pay_id` DESC ";
            break; 
            case '11':
              // Organzation Lead
              $customerselect = "SELECT * FROM `payment_request` WHERE (`created_by` = '$logged_admin_id' OR `org_name`= '$logged_admin_org') ORDER BY `pay_id` DESC ";
            break;
        }
