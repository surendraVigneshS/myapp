<table class="display payment-table" id="">
    ...
    <tbody>
        ...
        switch ($logged_admin_role) {
            case '6':
              //Employee
              $customerselect = "SELECT * FROM `payment_request` WHERE `orglead_approval` = 2 AND `created_by` = '$logged_admin_id' AND `user_cancel` = 0 AND `first_approval` = 1 ORDER BY `pay_id` DESC ";
            break;
            case '3':
            case '10':
              //Team Leader
              $customerselect = "SELECT * FROM `payment_request` WHERE `orglead_approval` = 2 AND (`team_leader` = '$logged_admin_id' OR `created_by` = '$logged_admin_id') AND `user_cancel` = 0 AND `first_approval` = 1 ORDER BY `pay_id` DESC ";
            break;
            case '4':
              //Finance  
              $customerselect = "SELECT * FROM `payment_request` WHERE `orglead_approval` = 2 AND (`raised_by` = 3 OR `created_by` = '$logged_admin_id') AND `user_cancel` = 0 AND `first_approval` = 1 ORDER BY `pay_id` DESC ";
            break;
            case '5':
              //Purchase Team 
              $customerselect = "SELECT * FROM `payment_request` WHERE `orglead_approval` = 2 AND `created_by` = '$logged_admin_id' AND `user_cancel` = 0 AND `first_approval` = 1 ORDER BY `pay_id` DESC ";
            ...
            case '7':
              //Purchase Lead  
              $customerselect = "SELECT * FROM `payment_request` WHERE `orglead_approval` = 2 AND (`raised_by` = 2 OR `created_by` = '$logged_admin_id') AND `user_cancel` = 0 AND `first_approval` = 1 ORDER BY `pay_id` DESC ";
            break;
            case '1':
              //Admin  
              $customerselect = "SELECT * FROM `payment_request` WHERE `orglead_approval` = 2 AND `user_cancel` = 0 AND `first_approval` = 1 ORDER BY `pay_id` DESC ";
            break;
            case '8':
              //Accounts  
              $customerselect = "SELECT * FROM `payment_request` WHERE `orglead_approval` = 2 AND `user_cancel` = 0 AND `first_approval` = 1 ORDER BY `pay_id` DESC ";
            ...
            case '9':
              //AGEM Accounts  
              $customerselect = "SELECT * FROM `payment_request` WHERE `orglead_approval` = 2 AND (`raised_by` = 3 OR `raised_by` = 1 OR `created_by` = '$logged_admin_id') AND `user_cancel` = 0 AND `first_approval` = 1  AND ( `org_name`=2 OR `org_name`='Agem') ORDER BY `pay_id` DESC ";
            break;
            case '11':
              // Organzation Lead
              $customerselect = "SELECT * FROM `payment_request` WHERE `orglead_approval` = 2 AND (`created_by` = '$logged_admin_id' OR `org_name`= '$logged_admin_org') AND `user_cancel` = 0 AND `first_approval` = 1 ORDER BY `pay_id` DESC ";
            break;
        }
        ...
        <tr style="color: <?php echo $orgColor ?>;" class="fw-500">
          ...
          <td style="color: <?php echo $orgColor ?>;" class="fw-500"><?php echo $orgName  ?></td>
          ...
          <td>
            <?php if ($firstApproval == 1 && $orgLeadApproval == 2) { ?>
              <label class="badge badge-info"> Pending  </label>
            <?php } ?>
            By- <?php if($orgName == 'SVCGPL'){ echo 'Accounts Team';}else{ echo 'Org Lead'; } ?>
          </td>
          <td>
