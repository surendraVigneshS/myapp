<table class="display payment-table" id="">
    ...
    <tbody>
        ...
        switch ($logged_admin_role) {
            case '6':
              //Employee
              $customerselect = "SELECT * FROM `payment_request` WHERE `created_by` = '$logged_admin_id' AND `user_cancel` = 0 AND `payment_against` = 3 AND `fourth_approval` = 1 ORDER BY `pay_id` DESC ";
            break;
            case '3':
              //Team Leader
              $customerselect = "SELECT * FROM `payment_request` WHERE (`team_leader` = '$logged_admin_id' OR `created_by` = '$logged_admin_id') AND `payment_against` = 3  AND `fourth_approval` = 1 ORDER BY `pay_id` DESC";
            break;
            case '4':
              //Finance  
              $customerselect = "SELECT * FROM `payment_request` WHERE `payment_against` = 3 AND `fourth_approval` = 1 ORDER BY `pay_id` DESC ";
            break;
            case '5':
              //Purchase Team  
              $customerselect = "SELECT * FROM `payment_request` WHERE `created_by` = '$logged_admin_id' AND `payment_against` = 3 AND `fourth_approval` = 1 ORDER BY `pay_id` DESC ";
            break;
            case '7':
              //Purchase Lead  
              $customerselect = "SELECT * FROM `payment_request` WHERE `raised_by` = 2 AND `payment_against` = 3 AND `fourth_approval` = 1 ORDER BY `pay_id` DESC ";
            break;
            case '1':
              //Admin  
              $customerselect = "SELECT * FROM `payment_request` WHERE `payment_against` = 3 AND `fourth_approval` = 1 ORDER BY `pay_id` DESC ";
            ...
            case '9':
              //AGEM Accounts  
              $customerselect = "SELECT * FROM `payment_request` WHERE `payment_against` = 3 AND `fourth_approval` = 1 AND `org_name` = 2 ORDER BY `pay_id` DESC ";
            break;
            case '11':
              // Organzation Lead
              $customerselect = "SELECT * FROM `payment_request` WHERE (`created_by` = '$logged_admin_id' OR `org_name`= '$logged_admin_org') AND `payment_against` = 3 AND `fourth_approval` = 1  ORDER BY `pay_id` DESC ";
            break;
        }
