<table class="display payment-table" id="">
    ...
    <?php
        $slno = 1; 
        $customerselect = "SELECT * FROM `purchase_request` WHERE  `already_purchased` = 1  ORDER BY `pur_id` DESC";
        switch ($logged_admin_role) {
            case '6':
                // user
                $customerselect = "SELECT * FROM `purchase_request` WHERE `already_purchased` = 1  AND `created_by` = '$logged_admin_id'  ORDER BY `pur_id` DESC";
                $editlink = "";
                break;
            case '3':
                // TL
                $customerselect = "SELECT * FROM `purchase_request` WHERE  (`created_by` = '$logged_admin_id' OR `team_leader` = '$logged_admin_id') AND  `already_purchased` = 1 ORDER BY `pur_id` DESC";
                $editlink = "";
                break; 
            case '9':
                // TL
                $customerselect = "SELECT * FROM `purchase_request` WHERE  (`org_name` = '2' OR `org_name` = 'Agem') AND  `already_purchased` = 1 ORDER BY `pur_id` DESC";
