<table class="display payment-table" id="">
    ...
    <tbody>
        ...
        switch ($logged_admin_role) {
            case '6':
              //Employee
              $customerselect = "SELECT * FROM `payment_request` WHERE `created_by` = '$logged_admin_id' AND `user_cancel` = 0 AND `first_approval` = 0 ORDER BY `pay_id` DESC ";
            break;
            case '3':
            case '10':
              //Team Leader // Reminder Team
              $customerselect = "SELECT * FROM `payment_request` WHERE (`team_leader` = '$logged_admin_id' OR `created_by` = '$logged_admin_id') AND `user_cancel` = 0  AND `first_approval` = 0 ORDER BY `pay_id` DESC";
            break;
            case '4':
              //Finance
              $customerselect = "SELECT * FROM `payment_request` WHERE (`raised_by` = 3 OR `created_by` = '$logged_admin_id') AND `user_cancel` = 0 AND `first_approval` = 0 ORDER BY `pay_id` DESC ";
              
            break;
            case '5':
              //Purchase Team 
              $customerselect = "SELECT * FROM `payment_request` WHERE `created_by` = '$logged_admin_id' AND `first_approval` = 0 ORDER BY `pay_id` DESC ";
            break;
            case '7':
              //Purchase Leader  
              $customerselect = "SELECT * FROM `payment_request` WHERE (`raised_by` = 2 OR `purchase_payment` = 1 OR `created_by` = '$logged_admin_id') AND `first_approval` = 0 ORDER BY `pay_id` DESC ";
            break;
            case '1':
              //Admin  
              $customerselect = "SELECT * FROM `payment_request` WHERE `first_approval` = 0 AND `user_cancel` = 0   ORDER BY `pay_id` DESC ";
            break;
            case '8':
              //Accounts  
              $customerselect = "SELECT * FROM `payment_request` WHERE (`created_by` = '$logged_admin_id' OR `raised_by` = 3) AND `user_cancel` = 0 AND `first_approval` = 0 ORDER BY `pay_id` DESC ";
            break;
            case '9':
              //AGEM Accounts  
              $customerselect = "SELECT * FROM `payment_request` WHERE (`raised_by` = 3 OR `raised_by` = 1 OR `created_by` = '$logged_admin_id') AND `user_cancel` = 0 AND `first_approval` = 0 AND ( `org_name`=2 OR `org_name`='Agem') ORDER BY `pay_id` DESC ";
            break; 
            case '11':
               // Organzation Lead
               $customerselect = "SELECT * FROM `payment_request` WHERE (`created_by` = '$logged_admin_id' OR `org_name` = '$logged_admin_org') AND `user_cancel` = 0  AND `first_approval` = 0 ORDER BY `pay_id` DESC ";
            break;
        }
