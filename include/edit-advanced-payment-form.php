...
<div class="mb-3 row">
    ...
    <div class="col-sm-8">
      <input class="form-control" type="text" id="companyName" name="companyName" value="<?php echo $company_name ?>" required>
    </div>
</div>
<div class="mb-3 row">
...
<?php if(noneditbalance($dbconnection,$payCode) == 1){ ?>
<div class="mb-3 row">
    <label class="col-sm-4 col-form-label" for="">Total amount</label>
    <div class="col-sm-8">
      <input class="form-control" type="text" id="totalamount" value="<?php echo $amount ?>"  name="amount" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="9" <?php if(noneditbalance($dbconnection,$payCode) != 1){ echo "readonly";  } ?>>
...
<div class="mb-3 row">
    ...
    <div class="col-sm-4">
        ...
        <input class="form-control" type="text" id="advencAmount" name="advencAmount" value="<?php echo $advance ?>" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="9" required>
    </div>
    <div class="col-sm-4">
        <label for="balanceAmount">Balance amount</label>
        <input type="hidden" name="nonbalanceAmount" id="nonbalanceAmount" value="0">
...
<?php }else{
    $customerselects = "SELECT * FROM `payment_request` WHERE `pay_code` = '$payCode' ORDER BY `pay_id` DESC LIMIT 1";
    $custoemrquerys = mysqli_query($dbconnection, $customerselects);
    if (mysqli_num_rows($custoemrquerys) > 0) {
      if ($rows = mysqli_fetch_array($custoemrquerys)) {
        $toalamounts = $rows['amount'];
        $noadvance = $row['advanced_amonut'];
        $nobalance = $rows['balance_amount'];      
?>
<input type="hidden" name="" id="totalamount" value="<?php if($nobalance != 0){ echo $nobalance + $noadvance; }else{ echo $advance; } ?>">
<div class="mb-3 row">
    <label class="col-sm-4 col-form-label" for="">Total amount</label>
    <div class="col-sm-8">
      <input class="form-control" type="text" value="<?php echo $toalamounts ?>"  name="amount" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="9" <?php if(noneditbalance($dbconnection,$payCode) != 1){ echo "readonly";  } ?>>
      <div class="invalid-feedback">Total Amount should be less than  Advance Amount</div>
    </div>
</div>
<?php } } ?>
<div class="mb-3 row">
    ...
    <div class="col-sm-4">
        <label for="balanceAmount">Balance amount</label>
        <input type="hidden" name="nonbalanceAmount" id="nonbalanceAmount" value="<?php if($nobalance != 0){ echo $nobalance + $noadvance; }else{ echo $balance;} ?> ">
        <input type="text" class="form-control" id="balanceAmount" name="balanceAmount" value="<?php echo $balance ?>" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="9" readonly>
    </div>
</div>
 <?php } ?>
...
<div class="mb-3 row">
    ...
    <div class="col-sm-8">
        <textarea id="totalamountWords" name="amountWords" required rows="3" class="form-control"><?php echo $amount_words ?></textarea>
    </div>
</div>
<div class="mb-3 row">
...
<?php if(($logged_admin_role == 6 || $logged_admin_role == 5 || $logged_admin_role == 7 || $logged_admin_role == 1 || $logged_admin_role == 4 || $logged_admin_role == 9) && $firstApproval == 1){ ?>
<div class="card-footer text-center">
    <button class="btn btn-primary" type="submit" name="approvepaymentTeamLeader" id="approvepaymentTeamLeader">Save Changes</button>
</div>
<?php } if($logged_admin_role == 8 && $secondApproval) { ?>
<div class="card-footer text-center">
    <button class="btn btn-primary" type="submit" name="approvepaymentTeamLeader" id="approvepaymentTeamLeader">Save Changes</button>
</div> 
<?php } if($logged_admin_role == 3 && $firstApproval == 1){ ?>
<div class="card-footer text-center">
    <button class="btn btn-primary" type="submit" name="approvepaymentTeamLeader" id="approvepaymentTeamLeader">Update Changes</button>
</div>
<?php } ?>