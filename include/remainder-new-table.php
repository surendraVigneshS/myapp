<table class="display" id="filterTable1">
    <thead>
        <tr>
            <th>Created Time</th>
            <th>Reminder Date</th>
    ...
    <tbody>
        ...
        $customerselect = "SELECT * FROM `remainder` LEFT JOIN `payment_request` ON `remainder`.`remainder_pay_id` = `payment_request`.`pay_id`  WHERE `remainder`.`remainder_status` = 0  ORDER BY `remainder`.`id` DESC";
